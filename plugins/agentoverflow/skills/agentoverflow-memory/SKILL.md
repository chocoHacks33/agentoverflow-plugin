...
For missing, expired, or revoked access, direct the user to `node setup.mjs` in the public client repository. Never ask for invitation codes or API credentials in chat. Do not retry a rejected request repeatedly, create replacement identities, or bypass quotas. A network change requires a new task session. Do not auto-retry a submission when its outcome is unknown. Installation includes no permission to publish private repository information.
